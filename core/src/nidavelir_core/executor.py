...
from nidavelir_core.logging import configure_logging, log_context
...
def _heartbeat(attempt_id: UUID, task_id: UUID, owner: str, stopped: Event) -> None:
    settings = get_settings()
    interval = max(1, settings.executor_heartbeat_seconds)
    with log_context(task_id=task_id, attempt_id=attempt_id, stage="executor-heartbeat"):
        while not stopped.wait(interval):
            try:
                ...
                logger.warning("failed to renew executor lease", exc_info=True)


def _run_claimed_attempt(attempt_id: UUID, task_id: UUID, owner: str) -> None:
    stopped = Event()
    heartbeat = Thread(
        target=_heartbeat,
        args=(attempt_id, task_id, owner, stopped),
        daemon=True,
    )
    heartbeat.start()
    with log_context(
        ...
        try:
            # PREPARING work can be reclaimed after a dead executor's lease expires.
            # Clean only resources for this attempt, never the installation globally.
            cancel_attempt_resources(attempt_id)
            logger.info("executing claimed attempt")
            ...
            try:
                ...
                logger.warning("failed to release executor lease", exc_info=True)
        ...
        with SessionLocal() as session:
            ...
            for attempt in list_stale_running_attempts(session):
                with log_context(
                    ...
                    logger.warning("recovering stale running attempt")
                    ...
                    try:
                        ...
                        if task.state in {
                            ...
                            tasks.transition(
                                ...
                                reason=(
                                    "executor lease expired; previous attempt was interrupted"
                                ),
                            )
                    except InvalidTaskTransition:
                        logger.warning("could not recover task for stale attempt")
    except SQLAlchemyError:
...
def run() -> None:
    ...
    owner = _executor_id()
    active: dict[Future[None], tuple[UUID, UUID]] = {}
    ...
    with ThreadPoolExecutor(
        ...
        while True:
            ...
            for future, identity in list(active.items()):
                if future.done():
                    active.pop(future, None)
                    attempt_id, task_id = identity
                    try:
                        future.result()
                    except Exception:
                        with log_context(
                            ...
                            logger.exception("executor future failed")
            ...
            for _ in range(max(0, available)):
                ...
                active[future] = (attempt.id, attempt.task_id)
